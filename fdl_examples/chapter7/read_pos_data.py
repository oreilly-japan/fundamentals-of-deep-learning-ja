...
if not os.path.isdir("data/word2vecdb"):
    ...
    with open("data/pos_data/pos.train.txt") as f:
        ...
        train_dataset_raw = [
            e.split() for e in train_dataset_raw
            if len(e.split()) > 0
        ]
    ...
    while counter < len(train_dataset_raw):
        ...
        if counter < len(train_dataset_raw) - 1:
            ...
            if ((pair[0] + "_" + next_pair[0] in model)
                and (pair[1] == next_pair[1])):
                train_dataset.append([
                    pair[0] + "_" + next_pair[0],
                    pair[1]
                ])
                counter += 2
                continue
    ...
    for word in dataset_vocab:
        ...
        if word in model:
            db.Put(word.encode(), model[word])
        elif word in nonmodel_cache:
            db.Put(word.encode(), nonmodel_cache[word])
        else:
            print(word)
            nonmodel_cache[word] = np.random.uniform(
                -0.25,
                0.25,
                300
            ).astype(np.float32)
    ...
    with open("data/pos_data/pos.train.processed.txt") as f:
        ...
        train_dataset = [
            element.split() for element in train_dataset
    ...
    with open("data/pos_data/pos.test.processed.txt") as f:
        ...
        test_dataset = [
            element.split() for element in test_dataset
            if len(element.split()) > 0
        ]
...
class POSDataset():
    def __init__(self, db, dataset, tags_to_index, get_all=False):
        ...
        for pair in dataset:
            self.inputs.append(
                np.frombuffer(db.Get(pair[0].encode()), dtype=np.float32)
            )
    ...
    def minibatch(self, size):
        ...
        return np.array(batch_inputs), np.array(batch_tags)
