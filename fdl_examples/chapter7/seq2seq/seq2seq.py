...
def attention_decoder(
  ...
  if attention_states.get_shape()[2].value is None:
    raise ValueError(
      "Shape[2] of attention_states must be known:"
...
def embedding_attention_decoder(
  ...
  with variable_scope.variable_scope(
    scope or "embedding_attention_decoder",
    dtype=dtype
  ) as scope:
...
def embedding_attention_seq2seq(
  ...
  """
  with variable_scope.variable_scope(
    scope or "embedding_attention_seq2seq",
    dtype=dtype
  ) as scope:
    dtype = scope.dtype
    ...
    if output_projection is None:
      cell = rnn_cell.OutputProjectionWrapper(
        cell, 
        num_decoder_symbols
      )
