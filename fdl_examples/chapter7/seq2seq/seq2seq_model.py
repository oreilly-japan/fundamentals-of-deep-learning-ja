...
class Seq2SeqModel(object):
  ...
  def __init__(
    ...
    for i in xrange(buckets[-1][0]):  # Last bucket isthe biggest one.
  ...
  def step(self, session, encoder_inputs, decoder_inputs,
    ...
    input_feed[last_target] = np.zeros(
      [self.batch_size],
      dtype=np.int32
    )
    ...
    if not forward_only:
      output_feed = [
        self.updates[bucket_id],  # Update Op that SGD
