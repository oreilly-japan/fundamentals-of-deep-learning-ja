...
def create_model(session, forward_only):
  ...
  and initialize or load parameters in session.
  """
...
def train():
  ...
  with tf.Session() as sess:
    ...
    while True:
      ...
      step_time += time_elapsed / FLAGS.steps_per_checkpoint
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        perplexity = (
          math.exp(float(loss)) if loss < 300
          else float("inf")
        )
        print (
          "global step %d learning rate %.4f"
        ...
        # last 3 times.
        if (
          len(previous_losses) > 2
          and loss > max(previous_losses[-3:])
        ):
        ...
        for bucket_id in xrange(len(_buckets)):
          ...
          eval_ppx = (
            math.exp(float(eval_loss)) if eval_loss < 300
            else float("inf")
          )
          print(
