...
class DQNAgent(object):
  def __init__(
    self, session, num_actions, learning_rate=1e-3, history_length=4,
    screen_height=84, screen_width=84, gamma=0.99
  ):
  ...
  def build_prediction_network(self):
    ...
    with tf.variable_scope("pred_network"):
      self.s_t = tf.placeholder(
        "float32",
        shape=[None, self.history_length, self.screen_height, self.screen_width],
        name="state"
      )
  ...
  def sample_and_train_pred(self, replay_table, batch_size):
    ...
    _, q_t, loss = self.session.run(
      [self.train_step, self.q_t, self.loss],
  ...
  def predict_action(self, state, epsilon_percentage):
    action_distribution = self.session.run(self.q_t, feed_dict={self.s_t: [state]})[0]
...
def main(argv):
  ...
  with tf.Session() as session:
    agent = DQNAgent(
      session=session, num_actions=num_actions, learning_rate=1e-4,
    ...
    for i in tqdm.tqdm(range(total_episodes)):
      ...
      if i % 50 == 0:
        ...
        print(
          "Reward Stats (min, max, median, mean): ", np.min(episode_rewards[-100:]),
          np.max(episode_rewards[-100:]), np.median(episode_rewards[-100:]),
          np.mean(episode_rewards[-100:])
        )
        print(
          "Global Stats (ep_percent, global_step_counter): ",
          ep_percent, global_step_counter
        )
        if q_t_list:
          print(
            "Qt Stats (min, max, median, mean): ", np.min(q_t_list[-1000:]),
            np.max(q_t_list[-100:]), np.median(q_t_list[-100:]), np.mean(q_t_list[-100:])
          )
        if ave_reward > 50.0:
          solved = True
          print("solved")
