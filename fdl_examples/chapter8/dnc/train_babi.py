...
def step_op(time, memory_state, controller_state, inputs, outputs):
    ...
    with tf.variable_scope("controller"):
        Xt = tf.concat([step_input, tf.reshape(r, [-1])], 0)
...
if __name__ == "__main__":
    ...
    with graph.as_default():
        ...
        _, _, _, _, final_outputs = tf.while_loop(
            ...
            body=step_op,
            loop_vars=(t, init_memory(N,W,R), initial_nn_state, unstacked_inputs, outputs_container),
        ...
        loss = tf.reduce_mean(
            loss_weights * tf.nn.softmax_cross_entropy_with_logits(logits=stacked_output, labels=target_output)
        )
        ...
        with tf.Session(graph=graph) as session:
            ...
            for i in range(iterations):
                ...
                if i % 100 == 0:
                    ...
                    tf.train.Saver().save(session, os.path.join(model_path, 'model.ckpt'), global_step=i)
            tf.train.Saver().save(session, os.path.join(model_path, 'model.ckpt'), global_step=i)
