...
if __name__ == "__main__":
    ...
    print(args)
    optimizer_name = args.optimizer[0]
    print("Optimizer: {}".format(optimizer_name))

    # Remove old summary and checkpoint
    if os.path.exists("optimizer_logs/{}".format(optimizer_name)):
        shutil.rmtree("optimizer_logs/{}".format(optimizer_name))
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("mlp_model"):
            ...
            train_op = training(cost, global_step, optimizer_name)
            eval_op = evaluate(output, y)
            summary_op = tf.summary.merge_all()
            saver = tf.train.Saver()
            sess = tf.Session()
            summary_writer = tf.summary.FileWriter(
                "optimizer_logs/{}".format(optimizer_name),
