...
def embedding_layer(x, embedding_shape):
    with tf.variable_scope("embedding"):
        embedding_init = tf.random_uniform(
            embedding_shape,
            -1.0,
            1.0
        )
        ...
        return (
            tf.nn.embedding_lookup(embedding_matrix, x),
            embedding_matrix
        )

def noise_contrastive_loss(embedding_lookup, weight_shape, bias_shape, y):
    with tf.variable_scope("nce"):
        nce_weight_init = tf.truncated_normal(
            weight_shape,
        ...
        nce_b = tf.get_variable(
            "b",
            initializer=nce_bias_init
        )

        total_loss = tf.nn.nce_loss(
            nce_W,
            nce_b,
            y, 
            embedding_lookup,
...
if __name__ == '__main__':
    with tf.Graph().as_default():
        with tf.variable_scope("skipgram_model"):
            ...
            e_lookup, e_matrix = embedding_layer(
                x,
                ...
                for minibatch in range(batches_per_epoch):
                    ...
                    if step % display_step == 0:
                        print(
                            "Elapsed:",
                            str(step),
                            "batches.Cost ={:.9f}".format(avg_cost)
                        )
                        avg_cost = 0
                    ...
                    if step % val_step == 0:
                        ...
                        for i in range(val_size):
                            ...
                            print_str = \
                                "Nearest neighbor of %s:" % val_word
                            for k in range(top_match):
                                match_word = data.reverse_dictionary[
                                    neighbors[k]
                                ]
