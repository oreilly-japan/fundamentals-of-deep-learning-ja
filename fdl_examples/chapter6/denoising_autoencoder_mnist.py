...
def corrupt_input(x):
    corrupting_matrix = tf.random_uniform(
        shape=tf.shape(x),
        minval=0,
        maxval=2,
        dtype=tf.float32
...
def layer(input, weight_shape, bias_shape, phase_train):
    ...
    return tf.nn.sigmoid(layer_batch_norm(
...
def decoder(code, n_code, phase_train):
    with tf.variable_scope("decoder"):
        ...
        with tf.variable_scope("hidden_2"):
            hidden_2 = layer(
                hidden_1,
                [n_decoder_hidden_1, n_decoder_hidden_2],
                [n_decoder_hidden_2],
                phase_train
            )
        ...
        with tf.variable_scope("hidden_3"):
            hidden_3 = layer(
                hidden_2,
                [n_decoder_hidden_2, n_decoder_hidden_3],
                [n_decoder_hidden_3],
                phase_train
            )
...
def loss(output, x):
    with tf.variable_scope("training"):
        l2 = tf.sqrt(
            tf.reduce_sum(
                tf.square(tf.subtract(output, x)),
                1
            )
        )
...
if __name__ == '__main__':
    ...
    n_code = args.n_code
    log_dir = "mnist_autoencoder_hidden_{}_logs/".format(n_code)
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("autoencoder_model"):
            ...
            phase_train = tf.placeholder(tf.bool)
            code = encoder(c_x, n_code, phase_train)
